...
class MLP:
  ...
  def build_graph(self):
    ...
    weights_1 = tf.get_variable(
        ...
        initializer = tf.random_normal_initializer(seed = 2024)
    )
...
def load_dataset():
    train_data_reader = DataReader(
        data_path = 'session-1/data/20news-bydate/tf-idf_train.txt',
        batch_size = 50,
        vocab_size = vocab_size
    )
    test_data_reader = DataReader(
        data_path = 'session-1/data/20news-bydate/tf-idf_test.txt',
        batch_size = 50,
        vocab_size = vocab_size
    )
